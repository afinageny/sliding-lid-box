...
function translateLength(v, direction, length) =
  [
    v[0] + direction[0] * length,
...
module boxLid(
  ...
  difference() {
    ...
    bevel(start=[width + diffEps, length + doveTailWidth, thikness], direction=[-1, 0], width=doveTailWidth, height=thikness, length=width + 2 * diffEps);
